    ...
    with tab1:
        ...
        if not st.session_state.match_squad.empty:
            ...
            <div class="section-box">
                <div style="color:#FFFF5722; font-weight:bold; font-size:18px; margin-bottom: 10px;">LINEUPS</div>
    ...
    with tab2:
        if not st.session_state.match_squad.empty:
            ...
            if st.session_state.match_simulation:
                ...
                # --- 📸 GENERATE COMMENTARY IMAGE (OPTIMIZED) ---
                # 1. Calculate required height
                wrapped_lines = textwrap.wrap(st.session_state.match_simulation, width=50)
                wrapped_text = "\n".join(wrapped_lines)
                n_lines = len(wrapped_lines)
                # Base height + dynamic height per line, capped between 6 and 12 inches
                dynamic_h = min(max(6, 2.5 + (n_lines * 0.35)), 12)

                comm_fig = plt.figure(figsize=(8, dynamic_h)) # Use dynamic height
                ...
                # Title & Line
                ax_c.text(0.5, 0.92, "🎙️ MATCH COMMENTARY", color='#FF5722', fontsize=22, ha='center', weight='bold', fontfamily='sans-serif')
                ax_c.plot([0.1, 0.9], [0.89, 0.89], color='#FF5722', lw=2)
                
                # Body Text (Positioned slightly lower)
                ax_c.text(0.5, 0.85, wrapped_text, color='white', fontsize=12, ha='center', va='top', fontfamily='monospace')
